...
namespace SkyApm.Diagnostics.Grpc.Client
{
    public class ClientDiagnosticProcessor
    {
            ...
            where TResponse : class
        {
            ...
            var spanOrSegment = _tracingContext.CreateExit($"{host}{grpcContext.Method.FullName}", host, carrierHeader);
            spanOrSegment.Span.SpanLayer = SpanLayer.RPC_FRAMEWORK;
            spanOrSegment.Span.Component = Components.GRPC;
            spanOrSegment.Span.Peer = new StringOrIntValue(host);
            spanOrSegment.Span.AddTag(Tags.GRPC_METHOD_NAME, grpcContext.Method.FullName);
