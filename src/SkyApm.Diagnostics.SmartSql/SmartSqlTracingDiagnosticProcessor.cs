...
using ExecutionContext = SmartSql.ExecutionContext;
...
public class SmartSqlTracingDiagnosticProcessor : ITracingDiagnosticProcessor
{
    ...
    private static void AddConnectionTag(SegmentContext context, DbConnection dbConnection)
    {
        ...
        {
            context.Span.Peer = new(dbConnection.DataSource);
        }
        if (dbConnection.Database != null)
        {
            _ = context.Span.AddTag(Common.Tags.DB_INSTANCE, dbConnection.Database);
        }
    }
    ...
    {
        _ = CreateSmartSqlLocalSegmentContext("BeginTransaction");
    }
    ...
    public void AfterDbSessionInvoke([Object] DbSessionInvokeAfterEventData eventData)
    {
        var context = _localSegmentContextAccessor.Context;
        if (context is null) return;
        _ = context.Span.AddTag("from_cache", eventData.ExecutionContext.Result.FromCache);
        var resultSize = eventData.ExecutionContext.Result.IsList
            ? (eventData.ExecutionContext.Result.GetData() as ICollection)?.Count
            : 1;
        _ = context.Span.AddTag("result_size", resultSize?.ToString());
        _tracingContext.Release(context);
    }
    ...
    {
        var context = CreateSmartSqlLocalSegmentContext(eventData.Operation);
        if (eventData.ExecutionContext.Request.RealSql is not null)
        {
            _=context.Span.AddTag(Common.Tags.DB_STATEMENT, eventData.ExecutionContext.Request.RealSql);
        }
    }
