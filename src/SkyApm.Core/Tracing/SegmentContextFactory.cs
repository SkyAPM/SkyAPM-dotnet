...
public class SegmentContextFactory : ISegmentContextFactory
{
    ...
    private bool GetSampled(ICarrier carrier, string operationName)
    {
        ...
        if (carrier.HasValue)
        {
            samplingContext = new(operationName, carrier.NetworkAddress, carrier.EntryEndpoint,
                carrier.ParentEndpoint);
        }
        else
        {
            samplingContext = new(operationName, default(StringOrIntValue), default(StringOrIntValue),
        ...
        StringOrIntValue peer = default(StringOrIntValue))
    {
        ...
        var sampledContext = new SamplingContext(operationName, peer, new(operationName),
            default(StringOrIntValue));
        var sampler = _samplerChainBuilder.Build();
